...
use Modules\Roles\Http\Requests\AssignUserRequest;
use Modules\Roles\Http\Requests\CreateRoleRequest;
use Modules\Roles\Http\Requests\RemoveUserRequest;
...
class RolesController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            if ($request->has('onlyTrashed') && $request->onlyTrashed) {
                $records = Role::onlyTrashed()->latest()->paginate(9);
            } else {
                $records = Role::latest()->paginate(9);
            }
            ...
            if ($records->count() > 0) {
                return response()->json(
                    [
                        ...
                        'data' => RoleResource::collection($records)->response()->getData(true)
                    ],200);
            } else {
    ...
    public function store(CreateRoleRequest $request)
    {
        try {
            ...
            $permission_store = $store->syncPermissions($request->permissions);
            if ($store && $permission_store) {
                DB::commit();
    ...
    public function show($id)
    {
        try {
            $record = Role::find($id);
            if ($record) {
                return response()->json(['message' => __('Changed Successfully'), 'data' => RoleResource::make($record)], 200);
            } else {
                return response()->json(['message' => __('Model not found'), 'data' => ''], 400);
            }
        } catch (Throwable $e) {
        ...
        try {
            ...
            if ($record) {
                $update = $record->update($request->except('_method', '_token'));
                    ...
                    $permission_store = $record->syncPermissions($request->permissions);
                if ($update && $permission_store) {
                    DB::commit();
    ...
    public function restore($id)
    {
        try {
            DB::beginTransaction();
            $record = Role::onlyTrashed()->find($id);
            if ($record) {
    ...
    public function assignUser(AssignUserRequest $request)
    {
        try {
            ...
            if ($record) {
                $assign = $record->assignRole($request->roles);
                if ($assign) {
                    DB::commit();
    ...
    public function removeUser(RemoveUserRequest $request)
    {
        try {
            ...
            $record = User::find($request->user_id);
            if ($record) {
                $assign = $record->removeRole($request->role);
                if ($assign) {
                    DB::commit();
                    return response()->json(['message' => __('Deleted Successfully'), 'data' => ''], 200);
                } else {
