...
class CategoriesController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            if ($request->has('onlyTrashed') && $request->onlyTrashed) {
                $records = Category::onlyTrashed()->where('parent_id', 0)->latest()->paginate(9);
            } else {
                $records = Category::latest()->where('parent_id', 0)->paginate(9);
            }
            if ($records->count() > 0) {
                return response()->json(
                    [
                        'message' => __('Returned Successfully'),
                        'data' => CategoryResource::collection($records)->response()->getData(true)
                    ],200);
            } else {
    ...
    {
        try {
            $store = Category::create($request->except('image'));
    ...
    public function show($id)
    {
        try {
            ...
            if ($record) {
                return response()->json(['message' => __('Changed Successfully'), 'data' => CategoryResource::make($record)],400);
            } else {
    ...
    {
        try {
            $record = Category::onlyTrashed()->find($id);
            if ($record) {
